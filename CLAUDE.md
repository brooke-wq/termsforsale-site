...
## Completed — April 26 2026 Weekly Team Digest System

Brooke wanted a process to push weekly site/system updates to the team
so everyone stays on the same page about what was fixed, what's new, and
what's next. Built it as a 3-channel digest (Email + Slack + Notion) on
the existing Paperclip cron infrastructure.

### What it does

Every Monday 7am AZ, a cron job on the Droplet pulls fresh data from 4
sources, synthesizes a 2-3 sentence TL;DR via Claude Haiku, and ships
the recap to:

1. **Email** — branded HTML email via GHL conversations API to a
   recipient list (default: Brooke only; expand via
   `DIGEST_RECIPIENTS` env var with comma-separated GHL contact IDs).
2. **Slack `#announcements`** — formatted Block Kit message via an
   incoming webhook (`SLACK_WEBHOOK_URL` env var).
3. **Notion "Track Changes Updates" page**
   (`c18090d675e783b187af8182ecf57920`) — appends a new section so the
   team has a permanent running log.

### Data sources

| Source | What it pulls | How |
|---|---|---|
| `CLAUDE.md` "Completed" sections | Anything dated within the last 7 days, with title + summary | Regex parse of `## Completed — <Month> <day> <year>` headers |
| Git log (last 7 days, main) | Every non-merge commit on main with hash/author/date/subject | `git log --since="7 days ago"` |
| Notion deals DB | Active inventory count, deals funded this week + $, new deals listed, status/type breakdowns | DB pagination + `Date Funded` / `Started Marketing` filters |
| GHL tag counters | Total opted-in buyers, lifetime alerts sent, viewed-deal events, INTERESTED/PASS replies, alerts-paused | One `/contacts/search` POST per tag (page=1, pageLimit=1, reads `meta.total`) |

### Files shipped

- **`termsforsale/netlify/functions/_digest.js`** (NEW) — pure data-
  gathering helper. Zero AI calls, zero outbound writes. Exports
  `gatherWeeklyData({days})` which returns `{window, completed,
  commits, notion, ghl, errors}`. Designed to be testable in isolation.
- **`termsforsale/netlify/functions/weekly-digest.js`** (NEW) — main
  function. Calls `gatherWeeklyData`, runs ONE Claude Haiku call for
  the TL;DR (~$0.001), builds 3 different format outputs (email HTML,
  Slack Block Kit, Notion blocks), and ships to all 3 channels in
  parallel via `Promise.all`. Fail-soft: a Slack webhook failure
  doesn't block the email or Notion delivery.
- **`jobs/weekly-digest-cron.js`** (NEW) — pm2-managed cron wrapper
  that requires the function and invokes its handler directly. Honors
  `DIGEST_LIVE` env var: if not `"true"`, runs in preview-only mode
  even when fired by cron (no sends, just logs the would-be content).
- **`netlify.toml`** — added `/api/weekly-digest` →
  `/.netlify/functions/weekly-digest` redirect.

### Modes

- **Preview** (default, safe): GET `/api/weekly-digest?test=true`
  returns the gathered data + TL;DR + first 600 chars of email HTML.
  Sends nothing. Use this to inspect the digest before going live.
- **Live**: requires both `DIGEST_LIVE=true` env var AND a
  request without `?test=true`. Cron always sends a non-test POST,
  so flipping `DIGEST_LIVE=true` enables Monday auto-delivery.

### Idempotency

File-based dedup via `jobs/sent-log.js` (same pattern as ceo-briefing,
weekly-synthesis). Key = `digest-<week-start-iso>`. If cron fires twice
in the same week (e.g. operator manually retriggers), the second run
returns `mode: "skipped"` and sends nothing.

### Cost

| Component | Cost |
|---|---|
| Claude Haiku TL;DR (~1500 input + 200 output tokens) | ~$0.001/run |
| GHL email send | $0 (existing plan) |
| Slack webhook | $0 |
| Notion API append | $0 |
| **Total** | **~$0.05/year** |

### Env vars to set on Netlify (Terms For Sale site)

| Var | Value | Notes |
|---|---|---|
| `ANTHROPIC_API_KEY` | (already set) | Required for TL;DR |
| `NOTION_TOKEN` | (already set) | Required for both deals pull + page append |
| `NOTION_DIGEST_PAGE_ID` | `c18090d675e783b187af8182ecf57920` | "Track Changes Updates" page |
| `SLACK_WEBHOOK_URL` | https://hooks.slack.com/services/... | Create a new Incoming Webhook in Slack pointed at `#announcements` |
| `DIGEST_RECIPIENTS` | `1HMBtAv9EuTlJa5EekAL` | Brooke's contact ID. Add more comma-separated when team grows. Defaults to `BROOKE_CONTACT_ID` if unset. |
| `DIGEST_LIVE` | `false` initially, then `true` after preview looks good | Master kill switch — leave `false` for the first 1-2 weeks while reviewing the format |

### Env vars to mirror on Paperclip Droplet (`/etc/environment`)

Same vars as Netlify. The cron wrapper invokes the function directly,
so it reads from the Droplet's environment, not Netlify's.

```
ANTHROPIC_API_KEY=<existing>
NOTION_TOKEN=<existing>
NOTION_DB_ID=a3c0a38fd9294d758dedabab2548ff29
NOTION_DIGEST_PAGE_ID=c18090d675e783b187af8182ecf57920
GHL_API_KEY=<existing>
GHL_LOCATION_ID=7IyUgu1zpi38MDYpSDTs
BROOKE_CONTACT_ID=1HMBtAv9EuTlJa5EekAL
...
pm2 start jobs/weekly-digest-cron.js --name weekly-digest --no-autorestart --cron "0 14 * * 1"
pm2 save
```

`0 14 * * 1` = every Monday at 14:00 UTC = **Monday 7:00 AM AZ** (no
DST in Arizona, so this is stable year-round).

### Slack incoming webhook setup (one-time, ~3 min)

1. https://api.slack.com/apps → **Create New App** → From scratch
2. Name: "Terms For Sale Digest". Workspace: Brooke's workspace.
3. **Features** → **Incoming Webhooks** → toggle ON
4. **Add New Webhook to Workspace** → pick `#announcements` → Allow
5. Copy the URL (starts with `https://hooks.slack.com/services/T...`)
6. Paste into Netlify env var `SLACK_WEBHOOK_URL` AND on the Droplet's
   `/etc/environment`

### Notion permission setup (one-time, ~30 sec)

The "Track Changes Updates" page must be shared with the existing
Notion integration that owns `NOTION_TOKEN`:

1. Open `https://www.notion.so/availabledeals/Track-Changes-Updates-c18090d675e783b187af8182ecf57920`
2. Click `•••` (top-right) → **Add connections** → pick the existing
   "Deal Pros" / Terms For Sale integration → confirm
3. Without this step, the Notion append will return 404
   "object_not_found".

### How to dry-run today

After deploying:
```bash
# Preview mode — safe, no sends
curl -s "https://termsforsale.com/api/weekly-digest?test=true" | jq

# When ready to flip on live (after setting DIGEST_LIVE=true on Netlify):
curl -X POST "https://termsforsale.com/api/weekly-digest"
```

The preview response includes `gatherErrors[]` so any source-level
problem (Notion auth, missing CLAUDE.md, etc.) shows up immediately.

### Known caveats

- **GHL tag counts are lifetime**, not week-only. The funnel section
  shows cumulative numbers with an explanatory footer. Week-over-week
  deltas would require a snapshot table — flagged in TODO as v2.
- **CLAUDE.md parsing relies on the `## Completed — <Month> <day>
  <year>` header convention** that's been used consistently since
  early April. If the format ever changes, the digest will silently
  show fewer items in the "What we shipped" section. Same parser is
  forgiving of em-dash vs. en-dash vs. hyphen.
- **Git log requires a non-shallow clone**. Netlify by default does a
  shallow clone, which means `getRecentCommits()` may return empty
  on Netlify-invoked runs. The Droplet has a full clone (it's the
  source of truth), so the cron-driven digest gets the full commit
  list. The Netlify-invoked preview will degrade to empty commits but
  won't error out.
- **First run will pull a lot of "What We Shipped" items** because
  CLAUDE.md has weeks of recent Completed sections. After the first
  Monday, only the last 7 days will appear.
...
0aa. **🔥 Wire up Weekly Team Digest to go live (built April 26)** — the system is built, just needs 3 setup steps:
   1. **Create Slack incoming webhook** for `#announcements` (https://api.slack.com/apps → Create New App → Incoming Webhooks → Add New Webhook to Workspace → pick `#announcements` → copy URL).
   2. **Share Notion page with integration**: open https://www.notion.so/availabledeals/Track-Changes-Updates-c18090d675e783b187af8182ecf57920 → `•••` → Add connections → pick the Deal Pros / Terms For Sale Notion integration.
   3. **Set Netlify env vars**: `SLACK_WEBHOOK_URL`, `NOTION_DIGEST_PAGE_ID=c18090d675e783b187af8182ecf57920`, `DIGEST_LIVE=false` (initially).
   4. **Mirror env vars on Paperclip Droplet** at `/etc/environment`, then register the cron:
      ```
      cd /root/termsforsale-site && git pull origin main
      pm2 start jobs/weekly-digest-cron.js --name weekly-digest --no-autorestart --cron "0 14 * * 1"
      pm2 save
      ```
   5. **Preview-test** with `curl "https://termsforsale.com/api/weekly-digest?test=true" | jq` — confirm TL;DR + numbers + shipped items look right.
   6. **Flip live** by setting `DIGEST_LIVE=true` on both Netlify and Droplet. First scheduled send: next Monday 7am AZ.
   See full session log "Completed — April 26 2026 Weekly Team Digest System" for details.
