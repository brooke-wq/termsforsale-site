...
exports.handler = async (event) => {
  ...
  // Auto-update Notion deal page with Slides URL (preferred: editable) + PDF URL fallback
  let notionPatched = false;
  let notionError = null;
  if (renderData.slidesWebViewLink || renderData.pdfWebViewLink) {
    try {
      const props = {};
      // Summary URL: prefer editable Slides link, fall back to PDF
      props['Summary URL'] = { url: renderData.slidesWebViewLink || renderData.pdfWebViewLink };
      // Analysis PDF URL: rich text field for the PDF link (separate from Slides)
      if (renderData.pdfWebViewLink) {
        props['Analysis PDF URL'] = { rich_text: [{ type: 'text', text: { content: renderData.pdfWebViewLink } }] };
      }
      const patchRes = await fetch(NOTION_BASE + '/pages/' + pageId, {
        method: 'PATCH',
      ...
      if (patchRes.ok) {
        notionPatched = true;
        console.log('[generate-pitch-deck] Notion Summary URL updated');
      } else {
        notionError = patchRes.status + ' ' + (await patchRes.text().catch(() => '')).slice(0, 200);
        console.warn('[generate-pitch-deck] Notion patch failed: ' + notionError);
      }
    } catch (e) {
      notionError = e.message;
      console.error('[generate-pitch-deck] Notion patch error:', e.message);
    }
  }
  ...
  return {
    ...
    body: JSON.stringify({
      ...
      dealId,
      address,
      htmlFileId: renderData.htmlFileId,
      htmlWebViewLink: renderData.htmlWebViewLink,
      pdfFileId: renderData.pdfFileId,
      pdfWebViewLink: renderData.pdfWebViewLink,
      slidesFileId: renderData.slidesFileId,
      slidesWebViewLink: renderData.slidesWebViewLink,
      notionPatched,
      notionError,
      tokensReplaced: Object.keys(tokens).length,
      tokensFilled: filledCount,
      unreplacedTokens: renderData.unreplacedTokens || []
    })
  };
};
